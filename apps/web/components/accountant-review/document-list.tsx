...
import { FlatAccountantDocument } from './types';
...
interface Props {
  ...
  companySlug: string;
  routeSegment: string;
  documents: FlatAccountantDocument[];
        ...
        <TableBody>
          {documents.map((doc) => (
            <TableRow key={doc.id}>
              <TableCell>
                <Link
                  href={`/${companySlug}/accountant-review/${routeSegment}/${doc.customerId}/${doc.id}`}
                  className="font-medium text-blue-700 hover:underline"
                >
                  {doc.number}
                </Link>
              </TableCell>
              <TableCell className="text-muted-foreground">{doc.customerName}</TableCell>
              <TableCell>
