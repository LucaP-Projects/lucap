...
export default function QualificationForm({
  ...
  const [numeroFacture, setNumeroFacture] = useState(
    existingQualification?.numeroFacture ?? documentNumber
  );
  const [montantHT, setMontantHT] = useState(
    existingQualification?.montantHT ?? documentAmount
  );
  const [tauxTVA, setTauxTVA] = useState(
    existingQualification?.tauxTVA ?? documentTaxRate ?? 19
  );
  ...
  const handleQualify = async (status: 'VALIDATED' | 'REJECTED') => {
    ...
    setError(null);
    try {
      const result = await qualifyDocument(documentType, documentId, {
        fournisseur,
        numeroFacture,
        montantHT,
        tauxTVA,
    ...
    <div className="flex h-full flex-col px-7 py-6">
      ...
      <div className="flex-1 space-y-5">
        ...
        {qualificationStatus && (
          ...
          >
            {qualificationStatus === 'VALIDATED'
              ? '✓ Document validé et commité avec succès'
              : '✗ Document passé / rejeté'}
          </div>
        )}
