...
type DelayedChargeSheetData = DelayedChargeWithRelations & {
...
interface DelayedChargeSheetProps {
  data: DelayedChargeSheetData | null;
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  isLoading: boolean;
}

export function DelayedChargeSheet({
  data,
  isOpen,
  onOpenChange,
  isLoading
}: DelayedChargeSheetProps) {
  ...
  const renderChargeItem = (item: any) => (
    <div
      key={item.id}
  ...
  const calculateSubtotal = () => {
    if (!data) return 0;
    // If we have discountAmount, we need to add it back to get the subtotal before discount
  ...
  return (
    <BaseSheet
      ...
      isLoading={isLoading}
      title={data ? `Delayed Charge #${data.number}` : 'Loading...'}
    >
      {data && (
        <div className="space-y-6">
          ...
          <div className="border-border bg-card/50 rounded-lg border p-4 shadow-sm">
            ...
            <div className="space-y-3">{data.items.map(renderChargeItem)}</div>
