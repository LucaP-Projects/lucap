...
interface DelayedCreditSheetProps {
  data: DelayedCreditData | null;
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  isLoading: boolean;
}

export function DelayedCreditSheet({
  data,
  isOpen,
  onOpenChange,
  isLoading
}: DelayedCreditSheetProps) {
  ...
  return (
    <BaseSheet
      ...
      isLoading={isLoading}
      title={data ? `Delayed Credit #${data.number}` : 'Loading...'}
    >
      {data && (
        <div className="space-y-6">
          ...
          <div className="border-border bg-card/50 rounded-lg border p-4 shadow-sm">
            ...
            <div className="space-y-3">{data.items.map(renderCreditItem)}</div>
