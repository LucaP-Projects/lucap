...
export function DelayedChargeForm({
  ...
  const [pendingSubmission, setPendingSubmission] =
    useState<DelayedChargeFormValues | null>(null);
  ...
  const handleSubmission = useCallback(
    async (data: DelayedChargeFormValues) => {
      try {
        setIsUploading(true);
        ...
        const result =
          mode === 'edit' && initialData
            ? await updateDelayedCharge(
                initialData.id,
                prepareFormData(data),
                settings,
                selectedColor
              )
            : await createDelayedCharge(
                prepareFormData(data),
                settings,
                selectedColor
              );
        ...
        router.push(`/${companySlug}/delayed-charge`);
        router.refresh();
      } catch (error) {
    ...
    <FormProvider {...formMethods}>
      <div className="relative flex h-full flex-col overflow-hidden">
        ...
        <div className="relative flex flex-1 overflow-hidden">
          ...
          <MemoizedSidebar paperType="Delayed Charge" />
        </div>
        <SubmitButton
          mode={mode}
          onSubmit={handleSubmit(onSubmit)}
          paperType="Delayed Charge"
        />
      </div>
    </FormProvider>
  );
