...
import {
  getCreditMemosPage,
  getCreditMemoStats
} from '@/components/dashboard/credit-memos/actions';
import CreditMemosPage from '@/components/dashboard/credit-memos/main';
...
export default async function Page({ searchParams }: PageProps) {
  ...
  const status =
    params.status === 'null'
      ? undefined
      : (params.status as CreditMemoStatus | undefined);
  ...
  return (
    <CreditMemosPage
      initialData={await getCreditMemosPage(page, 10, {
        status,
        search,
        dateFrom,
        dateTo
